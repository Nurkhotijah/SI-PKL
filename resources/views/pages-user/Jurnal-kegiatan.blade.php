...
<main class="p-6 overflow-y-auto h-full">
    ...
    <form action="{{ route('jurnal.upload', ['id' => $siswa->id]) }}" method="POST" enctype="multipart/form-siswa" class="inline-block">
        @csrf
        <input type="file" name="laporan_pkl" accept=".pdf" id="fileInput-{{ $siswa->id }}" class="hidden" onchange="this.form.submit()">
                ...
                <tbody>
                    @foreach($jurnal as $index => $siswa)
                        <tr>
                            ...
                            <td class="py-2 px-4 border-b text-center">
                                <a href="{{ route('jurnal-kegiatan.edit', ['id' => $siswa->id]) }}" class="bg-blue-500 text-white text-xs px-3 py-1 rounded shadow hover:bg-blue-600 transition duration-300 ease-in-out">
                                    <i class="fas fa-edit mr-1"></i> Edit
                                </a>
                                <form action="{{ route('jurnal-kegiatan.destroy', ['id' => $siswa->id]) }}" method="POST" class="inline-block ml-2" onsubmit="return confirm('Apakah Anda yakin ingin menghapus siswa ini?');">
                                    @csrf
                                    @method('DELETE')
